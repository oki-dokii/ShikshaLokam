...
const STAFFING_DATA = [
    { school: 'GHS Sector 4', teachers: 12, required: 15, status: 'deficit', value: -3 },
    { school: 'Primary School Old City', teachers: 8, required: 6, status: 'surplus', value: +2 },
    { school: 'KGBV Girls', teachers: 10, required: 10, status: 'balanced', value: 0 },
];
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <Card className="bg-[#0A0F1E]/80 border-blue-500/20 backdrop-blur-xl shadow-2xl overflow-hidden">
                ...
                <CardHeader className="flex flex-row items-center justify-between border-b border-white/5 pb-6">
                    ...
                    <Button variant="outline" className="border-blue-500/30 text-blue-400 hover:bg-blue-500/10 font-bold uppercase tracking-wide text-xs">
                        <Activity className="w-4 h-4 mr-2" /> Auto-Balance
                    </Button>
                </CardHeader>
                <CardContent className="pt-6">
                    <div className="space-y-3">
                        {STAFFING_DATA.map((item, i) => (
                            <div key={i} className="flex items-center justify-between p-5 bg-[#0F1629] rounded-xl border border-white/5 hover:border-blue-500/30 transition-all group">
                                ...
                                <div className="flex items-center gap-6">
                                    <Badge className={`px-4 py-1.5 text-xs font-bold tracking-wide uppercase ${item.status === 'deficit' ? 'bg-red-500/10 text-red-500 border border-red-500/20' :
                                        item.status === 'surplus' ? 'bg-orange-500/10 text-orange-400 border border-orange-500/20' :
                                            'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
                                        }`}>
                                        {item.status} {Math.abs(item.value) > 0 ? `(${item.value > 0 ? '+' : ''}${item.value})` : ''}
                                    </Badge>
                        ...
                        {ESCALATIONS.map((issue) => (
                            <div key={issue.id} className="p-4 bg-red-500/5 border border-red-500/10 rounded-xl hover:bg-red-500/10 transition-all cursor-pointer group relative overflow-hidden">
                                ...
                                <div className="flex gap-2 mt-4 pl-3">
                                    <Button size="sm" className="h-7 text-xs bg-red-600 hover:bg-red-500 text-white border-0 shadow-lg shadow-red-600/20">Resolve</Button>
