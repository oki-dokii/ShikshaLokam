...
uploadForm.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        currentDprId = data.dpr_id;

        // Display results
        displayJSON(data.summary);
...
async function sendChatMessage() {
    ...
    if (!message || !currentDprId) return;
    ...
    try {
        const response = await fetch(`/dpr/${currentDprId}/chat`, {
            method: 'POST',
            headers: {
...
loadHistoryBtn.addEventListener('click', async () => {
    if (!currentDprId) return;

    try {
        const response = await fetch(`/dpr/${currentDprId}/chat/history`);
        ...
        if (data.messages && data.messages.length > 0) {
            ...
            chatMessages.innerHTML = '<p style="color: #b0b0b0; text-align: center;">No chat history yet.</p>';
        }
